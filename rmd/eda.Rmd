...
salaries[order(SEASON), .SD[1], by = c('SIGNING DATE', 'merge_name')]
salaries$contract_stat_season <- year(as.Date(salaries$`SIGNING DATE`)) - 1

all_player_data <- merge(
    core_stats,
    salaries,
    by.y = c('merge_name', 'contract_stat_season'),
